...
class StreamSpikeNet(pl.LightningModule):
    ...
    def __init__(self, input_dim, context_dim, sr=16000,
                 ...
                 scnn_only=False, norm_type="lnorm",
                 no_pointwise=False):
        super().__init__()
        ...
        if norm_type == "lnorm":
            ...
            # BatchNorm1d folds into adjacent Conv at quantization time —
            # eliminates the FP32 island that LN forced on X-CUBE-AI.
            self.ln = nn.BatchNorm1d(N)
        elif norm_type == "none":
            self.ln = nn.Identity()
        else:
            raise ValueError(f"Unknown norm_type: {norm_type!r}")
        # ICECS latency config (Exp 12): drop the binarized bottleneck conv and
        # the mask conv entirely — LN output feeds the SCNN directly and the
        # sigmoid acts on the readout membrane. Requires N == B.
        self.no_pointwise = no_pointwise
        if no_pointwise:
            assert N == B, "no_pointwise requires N == B"
            self.proj = None
        else:
        ...
        self.mask = None if no_pointwise else nn.Conv1d(B, N, 1)
    ...
    def forward(self, batch):
        ...
        for feature_step in range(self.feature_steps):
            ...
            if self.no_pointwise:
                proj_events = x  # (b, B, 1) — dense LN output feeds SCNN directly
            else:
    ...
    def time_step_ops(self):
        ...
        proj_ops = np.prod(self.proj.weight.shape) if self.proj is not None else 0
        # sconv1d_ops = np.prod(self.repeats[0][0].dconv.weight.shape)
        mask_ops = np.prod(self.mask.weight.shape) if getattr(self, 'mask', None) is not None else 0
