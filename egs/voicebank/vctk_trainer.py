...
parser.add_argument('--norm_type', type=str, default='lnorm', choices=['lnorm', 'bnorm', 'none'],
                    help="Normalization between encoder and proj. 'bnorm' folds into adjacent "
                         "Conv at INT8 quant, eliminating the FP32 island that breaks X-CUBE-AI "
                         "INT8 propagation.")
parser.add_argument('--no_pointwise', action='store_true',
                    help='ICECS latency config: remove the binarized bottleneck conv '
                         'and the mask conv (LN output feeds SCNN directly; sigmoid '
                         'acts on readout membrane). Requires N == B.')
parser.add_argument('--exp_name', type=str, default='exp7',
                    help='Tag used for the per-epoch markdown log filename (results/<exp_name>_epoch_log.md).')
...
def start_func():
    ...
    class TestCallback(pl.Callback):
        ...
        def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
            ...
            # cast to fp32 so numpy/metrics don't choke on bf16
    ...
    epoch_log_path = os.path.join(script_dir,
                                  f'../../results/{args.exp_name}_epoch_log.md')

    class EpochLogger(pl.Callback):
        def __init__(self, log_path, exp_name, norm_type, L, stride):
            self.log_path = log_path
            self.exp_name = exp_name
            self.norm_type = norm_type
            self.L = L
            self.stride = stride
            self._best_val_sisnr = float('inf')

        def on_train_start(self, trainer, pl_module):
            with open(self.log_path, 'a') as f:
                f.write(f"\n## {self.exp_name} — norm={self.norm_type}, "
                        f"L={self.L}, stride={self.stride}"
                        f" (started {time.strftime('%Y-%m-%d %H:%M')})\n\n")
                f.write("| Epoch | val_loss | val_sisnr | notes |\n")
                f.write("|---|---|---|---|\n")

        def on_validation_epoch_end(self, trainer, pl_module):
            metrics = trainer.callback_metrics
            val_loss = metrics.get('val_loss')
            val_sisnr = metrics.get('val_sisnr')
            if val_loss is None or val_sisnr is None:
                return
            val_loss_f = float(val_loss)
            val_sisnr_f = float(val_sisnr)
            is_best = val_sisnr_f < self._best_val_sisnr
            if is_best:
                self._best_val_sisnr = val_sisnr_f
            note = "**new best**" if is_best else ""
            with open(self.log_path, 'a') as f:
                f.write(f"| {trainer.current_epoch} | {val_loss_f:.4f}"
                        f" | {val_sisnr_f:.4f} | {note} |\n")

    test_callback = TestCallback()
    epoch_logger = EpochLogger(epoch_log_path, args.exp_name, args.norm_type,
                               args.L, args.stride)
    checkpoint_callback = ModelCheckpoint(monitor=config.checkpoint.monitor,
                                          save_top_k=config.checkpoint.save_top_k,
                                          save_last=config.checkpoint.save_last,
                                          filename=config.checkpoint.filename)
    trainer = pl.Trainer(callbacks=[checkpoint_callback, epoch_logger, test_callback], **config.trainer)
