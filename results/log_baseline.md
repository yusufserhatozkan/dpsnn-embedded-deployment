...
- **Conv-TasNet fallback dropped** — not needed; ONNX export succeeded. Code since removed from repo.
...
Proceed to: train SCNN-only variant at N=B=H=128. The trained N=128 model becomes
Exp 5 (17.23 dB) → Exp 7 (17.41 dB) and is eventually deployed via the streaming
export wrapper. See `log_training.md` and `log_xcube_ai.md` for the rest of the chain.
...
Code was later removed from the repo (never trained).
...
| Parameters | **56,839** (sized to the then-estimated SCNN-only count; the actual SCNN-only model is 71,299 params) |
