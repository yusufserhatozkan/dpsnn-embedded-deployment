"""True-INT8 spike-aware quantization for DPSNN — deployment-ready variant.

Same spike-awareness as `quantize_spike_aware_correct.py` (membrane potentials
and spike outputs stay FP32), but Q/DQ pairs are placed so that X-CUBE-AI
can lower Conv/Gemm to native INT8 kernels at deploy time and keep
activations in INT8 between layers.

Difference from `quantize_spike_aware_correct.py`:
  - "correct" puts Q->DQ AFTER ReLU/Sigmoid -> Conv output is FP32 ->
    X-CUBE-AI keeps Conv in FP32 -> activations 1.47 MB (over 786 KB SRAM).
  - "true_int8" puts Q->DQ BETWEEN Conv and ReLU/Sigmoid -> Conv output is
    quantized -> X-CUBE-AI lowers Conv to INT8 with INT8 accumulator ->
    activations stored as INT8 between layers -> expected ~4× RAM shrink.

Math note: ReLU(Q->DQ(x)) ≈ Q->DQ(ReLU(x)) for any sane scale, since ReLU
is just clipping. Sigmoid(Q->DQ(x)) ≈ Q->DQ(Sigmoid(x)) is approximate but
the sigmoid output range is fully covered by the activation calibration.

SAFE convs: those followed (directly or through bias-Add) by ReLU/Sigmoid.
Spike-path convs feed Sub->Greater/GreaterOrEqual->Cast; we leave their
output FP32 because the spike threshold needs full dynamic range.

Usage
-----
  python export/quantize_spike_aware_true_int8.py \\
      ...
      --output_path export/dpsnn_scnn128_exp7_true_int8.onnx \\
      --n_calib 50
"""
...
def find_activation_output_tensors(
    ...
    """Find the CONV OUTPUT tensors that feed a Relu/Sigmoid (pre-activation).

    For X-CUBE-AI to lower a Conv to native INT8, the Conv's output must be
    immediately followed by a QuantizeLinear.  We therefore quantize the
    pre-activation tensor (Conv output), not the post-activation tensor.
    Returned lists track which group (ReLU or Sigmoid) each Conv output
    feeds — they get different quantization strategies:
      - "ReLU group" (signed): one shared asymmetric scale from global
        min/max across all time steps. Negative values are clipped by
        the downstream ReLU; capturing them in the INT8 range gives
        better quantization of the positive tail.
      - "Sigmoid group" (signed): one shared asymmetric scale from
        calibration. The downstream Sigmoid maps the full real line to
        (0, 1) so no clipping happens — the calibration just needs to
    ...
    for node in graph.node:
        ...
        is_safe = False
        # The tensor we want to quantize is the Conv output, i.e. the
        # tensor flowing INTO the Relu/Sigmoid.
        target_tensor = inp

        # Direct: Conv -> Relu/Sigmoid  -> target is the Conv output (`inp`)
        if producer.op_type in ('Conv', 'ConvTranspose', 'Gemm'):
            if (producer.name or '') in safe_node_names:
                is_safe = True

        # One hop: Conv -> Add (bias) -> Relu/Sigmoid -> target is the Add output
        # (still the pre-activation tensor; bias-add is just an offset).
        ...
        if is_safe:
            if node.op_type == 'Relu':
                relu_tensors.append(target_tensor)
            else:
                sigmoid_tensors.append(target_tensor)
...
def quantize_spike_aware_correct(
    ...
    # -- Find PRE-ACTIVATION Conv-output tensors of SAFE Conv nodes ----------
    # NOTE: this variant returns the Conv OUTPUT (= Relu/Sigmoid INPUT) so the
    # Q->DQ ends up between Conv and the activation, which is the layout
    # X-CUBE-AI can lower to native INT8.
    print("Finding Conv output tensors (pre-activation) of SAFE Conv nodes...")
    relu_tensors, sigmoid_tensors = find_activation_output_tensors(graph, safe_node_names)
    print(f"  Found {len(relu_tensors)} Conv->ReLU tensors, "
          f"{len(sigmoid_tensors)} Conv->Sigmoid tensors")
    ...
    # -- Calibrate BOTH groups: Conv output is signed, range varies per path -
    # All pre-activation tensors share ONE global scale per group (ReLU-bound
    # vs Sigmoid-bound), which matches X-CUBE-AI's per-layer scale model.
    relu_scale = np.float32(1e-8)
    relu_zp = np.int8(0)
    relu_global_max = 1.0
    if relu_tensors:
        print(f"Calibrating {len(relu_tensors)} Conv->ReLU tensors on {n_calib} samples...")
        relu_stats = calibrate(prep_path, relu_tensors, hdf5_path, n_calib)
        all_max_vals = [v[1] for v in relu_stats.values()]
        all_min_vals = [v[0] for v in relu_stats.values()]
        relu_global_min = float(np.percentile(all_min_vals, 100.0 - relu_percentile))
        relu_global_max = float(np.percentile(all_max_vals, relu_percentile))
        relu_scale, relu_zp = compute_asymmetric_scale_zp(relu_global_min, relu_global_max)
        abs_max = max(max(all_max_vals), abs(min(all_min_vals)))
        print(f"  Absolute max across all time steps: {abs_max:.4f}")
        print(f"  Range used: [{relu_global_min:.4f}, {relu_global_max:.4f}] "
              f"(percentile {relu_percentile})")
        print(f"  Shared Conv->ReLU scale: {relu_scale:.6f}, zp: {relu_zp}")

    sigmoid_scale = np.float32(1e-8)
    sigmoid_zp = np.int8(0)
    if sigmoid_tensors:
        print(f"Calibrating {len(sigmoid_tensors)} Conv->Sigmoid tensors on {n_calib} samples...")
        sig_stats = calibrate(prep_path, sigmoid_tensors, hdf5_path, n_calib)
        all_max_vals = [v[1] for v in sig_stats.values()]
        all_min_vals = [v[0] for v in sig_stats.values()]
        sig_global_min = float(np.percentile(all_min_vals, 100.0 - relu_percentile))
        sig_global_max = float(np.percentile(all_max_vals, relu_percentile))
        sigmoid_scale, sigmoid_zp = compute_asymmetric_scale_zp(sig_global_min, sig_global_max)
        abs_max = max(max(all_max_vals), abs(min(all_min_vals)))
        print(f"  Absolute max across all time steps: {abs_max:.4f}")
        print(f"  Range used: [{sig_global_min:.4f}, {sig_global_max:.4f}] "
              f"(percentile {relu_percentile})")
        print(f"  Shared Conv->Sigmoid scale: {sigmoid_scale:.6f}, zp: {sigmoid_zp}")

    # Legacy int8/int16 variables kept for the activation_bits=16 branch below.
    sigmoid_scale_int8 = sigmoid_scale
    sigmoid_zp_int8 = sigmoid_zp
    global_max = relu_global_max  # used by the INT16 branch
    sigmoid_scale_int16 = float(max(abs(sig_global_min), sig_global_max) / 32767.0
                                if sigmoid_tensors else 1.0 / 32767.0)
    ...
    if activation_bits == 16:
        ...
        print(f"Inserting INT8 Q->DQ nodes BETWEEN Conv and activation...")
        for i, tensor_name in enumerate(relu_tensors):
            insert_qdq_after_tensor(graph, tensor_name, float(relu_scale), int(relu_zp),
                                    suffix=f'pre_relu_{i}')
        for i, tensor_name in enumerate(sigmoid_tensors):
            insert_qdq_after_tensor(graph, tensor_name, float(sigmoid_scale_int8),
                                    int(sigmoid_zp_int8), suffix=f'pre_sig_{i}')
        print(f"  Inserted {len(relu_tensors)} Conv->ReLU Q->DQ pairs (shared scale)")
        print(f"  Inserted {len(sigmoid_tensors)} Conv->Sigmoid Q->DQ pairs (shared scale)")
